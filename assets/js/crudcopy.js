...
const widgets = {
  gallery: typeof cloudinary !== "undefined" ? cloudinary.createUploadWidget(
    {
      ...cloudinaryConfig,
      multiple: true,
      maxFiles: 10,
  ...
  cover: typeof cloudinary !== "undefined" ? cloudinary.createUploadWidget(
    {
      ...cloudinaryConfig,
      multiple: false,
      cropping: true,
...
function updateCoverImage(url) {
  ...
  elements.coverPreview.innerHTML = url ? `<img src="${url}" class="cover-preview-img">` : "";
  if (state.editId) {
    const vehicleItem = document.querySelector(`.vehicle-item[data-id="${state.editId}"]`);
    if (vehicleItem) {
...
async function loadVehicles() {
  try {
    ...
    const listHtml = (data || []).map((vehicle) => {
      ...
      return `
        <li class="vehicle-item ${vehicle.is_active ? "" : "sold"}" data-id="${vehicle.id}">
          <div class="vehicle-content">
            <div class="vehicle-info">
              <h3>${vehicle.title || "Sem título"}</h3>
              <p>Ano: ${vehicle.year || "----"} | Preço: ${formattedPrice}</p>
              ${vehicle.is_featured ? '<span class="featured-badge">★ Destaque</span>' : ''}
            </div>
            <div class="vehicle-image-container">
              ${vehicle.cover_image_url ? `<img src="${vehicle.cover_image_url}" class="vehicle-thumbnail">` : '<div class="no-image">Sem imagem</div>'}
              ${!vehicle.is_active ? '<div class="sold-banner">VENDIDO</div>' : ''}
            </div>
            <div class="vehicle-actions">
              <button type="button" class="btn-edit" data-id="${vehicle.id}">Editar</button>
              <button type="button" class="btn-delete" data-id="${vehicle.id}">Excluir</button>
            </div>
          </div>
        </li>
      `;
    ...
    document.querySelectorAll(".btn-edit").forEach((b) =>
      b.addEventListener("click", (e) => editVehicle(e.currentTarget.getAttribute("data-id")))
    );
    document.querySelectorAll(".btn-delete").forEach((b) =>
      b.addEventListener("click", (e) => deleteVehicle(e.currentTarget.getAttribute("data-id")))
    );
  } catch (error) {
...
window.editVehicle = async function (id) {
  try {
    ...
    if (data.brand && !document.querySelector(`#brand option[value="${data.brand}"]`)) {
    ...
    if (data.gallery_urls) {
      if (Array.isArray(data.gallery_urls)) {
        ...
        state.imageUrls = data.gallery_urls.split(",").map((u) => u.trim()).filter(Boolean);
      }
    }
    updateGalleryInput();
    renderGalleryPreviews();
    ...
    elements.form.querySelector('button[type="submit"]').textContent = "Atualizar";
...
function setupEventListeners() {
  ...
  if (widgets.gallery && elements.uploadGalleryBtn) {
    elements.uploadGalleryBtn.addEventListener("click", () => widgets.gallery.open());
  }
  if (widgets.cover && elements.uploadCoverBtn) {
    elements.uploadCoverBtn.addEventListener("click", () => widgets.cover.open());
  }
  ...
  elements.brand.addEventListener("change", function () {
    elements.otherBrand.style.display = this.value === "outro" ? "block" : "none";
  ...
  elements.form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const wasFeatured = !!elements.is_featured.getAttribute("data-was-featured");
      ...
      // brand final
      const brandValue = elements.brand.value === "outro" ? elements.otherBrand.value.trim() : elements.brand.value;
      ...
      const galleryArray = Array.isArray(state.imageUrls) && state.imageUrls.length > 0
        ? state.imageUrls.filter((u) => typeof u === "string" && u.trim().length > 0)
        : null;
      ...
      // validação mínima
      if (!carData.title || !carData.brand || !carData.model || isNaN(carData.year)) {
        alert("Preencha os campos obrigatórios corretamente");
        return;
      }

      if (state.editId) {
        const { error } = await supabase.from("cars").update(carData).eq("id", state.editId);
        if (error) throw error;
